...
export class AppController {
  ...
  getHello() {
    ...
    const val1 = BusinessRuleValidator.create<{ address?: number }>().addRule(
      'address',
    ...
    const basicUserValidator = BusinessRuleValidator.create<User>()
      ...
      .mustSatisfy(
        Specification.create<User>((item) => item.address !== undefined),
        'Address is required',
      );
    const user = {
      id: '1',
    ...
    policyFactory.register(
      'MY POLICY',
      basicUserValidator.toSpecification(),
      'POLICY_CODE',
      'POLICY_MESSAGE',
      (user) => user,
    );
