  ...
  private extractClassHandler(instance: object) {
    ...
    if (classMetadata) {
      ...
      if (instance['handle'] && typeof instance['handle'] === 'function') {
        this.eventBus.registerHandler(eventType, instance as IEventHandler<any>);
        
        console.log(`Zarejestrowano klasę obsługującą zdarzenie ${eventType.name}: ${instance.constructor.name}`);
      }
    }
  }
  ...
  private extractMethodsFromInstance(instance: object) {
    ...
    methodNames.forEach((methodName: string) => {
      ...
      if (metadata) {
        ...
        const handlerFn: EventHandlerFn<any> = instance[methodName].bind(instance);

        // Rejestruj metodę jako handler w eventBusie
        this.eventBus.subscribe(eventType, handlerFn);
        
        console.log(`Zarejestrowano handler metody dla ${eventType.name}: ${instance.constructor.name}.${methodName}`);
      }
    });
  }
}