...
import { IDomainEvent, IEventHandler, EventBusMiddleware } from '../../core';
import { safeRun } from '../../utils';

// Test events
class TestEvent implements IDomainEvent {
  eventType: string;
  constructor(public readonly payload: { value: string } = { value: 'default' }) {}
}
...
describe('InMemoryEventBus', () => {
  ...
  describe('Event publishing', () => {
    ...
    it('should do nothing when publishing to an event with no handlers', async () => {
      ...
      // Act
      const [_, result] = await safeRun(() => eventBus.publish(new TestEvent()));
    ...
    it('should continue execution if one handler throws', async () => {
      ...
      // Act
      const [error] = await safeRun(() => customEventBus.publish(new TestEvent()));
      ...
      const transformMiddleware: EventBusMiddleware = next => {
        return async event => {
          if (event instanceof TestEvent) {
            ...
            const transformedEvent = new TestEvent({ value: `transformed: ${event.payload.value}` });
  ...
  describe('Edge cases', () => {
    it('should handle undefined or null events gracefully', async () => {
      ...
      // Act & Assert
      // @ts-ignore - Deliberately passing invalid values for testing
    ...
    it('should handle malformed events', async () => {
      ...
      // Act & Assert
      // @ts-ignore - Deliberately creating a malformed event
    ...
    it('should work correctly with inheritance in event types', async () => {
      ...
      class DerivedTestEvent extends TestEvent {
        constructor(payload: { value: string; extra?: string } = { value: 'derived' }) {
          super(payload);
        }
      }
  ...
  describe('Performance considerations', () => {
    ...
    it('should handle high throughput of events', async () => {
      ...
      const promises = events.map(event => safeRun(() => eventBus.publish(event)));
