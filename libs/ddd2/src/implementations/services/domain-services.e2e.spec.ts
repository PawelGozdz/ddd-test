...
class CustomerNotifiedEvent extends DomainEventBase {
  readonly eventType = 'CustomerNotified';
...
// // Agregaty
// abstract class AggregateRoot<TId extends EntityId> implements IAggregateRoot<TId> {
...
class Order extends AggregateRoot {
  ...
  ship(shippingCode: string): void {
    ...
    this.apply(new OrderShippedEvent(this.getId().toString(), shippingCode));
  }
...
class NotificationService extends EventAwareDomainService implements IAsyncDomainService {
  ...
  async initialize(): Promise<void> {
    ...
    // Subskrybuj na zdarzenia, które wymagają notyfikacji
    this.eventBus.subscribe<OrderConfirmedEvent>(OrderConfirmedEvent, this.handleOrderConfirmed.bind(this));
    this.eventBus.subscribe<OrderShippedEvent>(OrderShippedEvent, this.handleOrderShipped.bind(this));
  }
  ...
  private handleOrderShipped(event: OrderShippedEvent): void {
    // W prawdziwej implementacji musielibyśmy pobrać dane klienta z repozytorium
    // Ale dla uproszczenia zakładamy, że już go mamy
    this.notifyCustomer(
      new EntityId('dummy-id', 'text'), // Tutaj powinno być ID klienta z zamówienia
...
describe('Domain Services - End-to-End Tests', () => {
  ...
  beforeEach(async () => {
    ...
    // Poczekaj na inicjalizację serwisów asynchronicznych
  ...
  describe('Obsługa błędów', () => {
    it('powinien obsłużyć brak produktu na stanie', async () => {
      ...
      try {
        await orderService.addOrderItem(order.getId(), watch.getId(), 2);
        fail('Powinna zostać zgłoszona wyjątek o braku towaru');
      } catch (error) {
  ...
  describe('Użycie ServiceRegistryBuilder', () => {
    it('powinien skonfigurować serwisy przy użyciu buildera', async () => {
      ...
      const customerServiceB = await registryBuilder
        ...
        .buildAndRegister();
      console.log('CustomerService built and registered', customerServiceB);
      ...
      // Sprawdź czy notyfikacje zostały wysłane
      const notifications = notificationServiceB.getSentNotifications();
      expect(notifications.length).toBe(1);
      expect(notifications[0].customerId).toBe(customer.getId().toString());
    });
  });
});
