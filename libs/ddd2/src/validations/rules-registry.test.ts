import { RulesRegistry, IRulesProvider, CoreRules, ISpecification, BusinessRuleValidator } from '../validations';
...
describe('CoreRules', () => {
  ...
  describe('Basic validation rules', () => {
    ...
    it('should validate maximum length', () => {
      // Arrange
      const validator = BusinessRuleValidator.create<TestData>()
        .apply(coreRules.maxLength('description', 20, 'Description is too long'));
      
      // Act
      const validResult = validator.validate({...validData, description: 'Short description'});
    ...
    it('should validate patterns', () => {
      // Arrange
      const validator = BusinessRuleValidator.create<TestData>()
        .apply(coreRules.pattern('name', /^[A-Z][a-z]+ [A-Z][a-z]+$/, 'Name must be in format "First Last"'));
      
      // Act
      const validResult = validator.validate(validData);
      const invalidResult = validator.validate({...invalidData, name: 'john doe'});
      
      // Assert
      expect(validResult.isSuccess).toBe(true);
      expect(invalidResult.isFailure).toBe(true);
      expect(invalidResult.error.errors[0].message).toBe('Name must be in format "First Last"');
    });
    ...
    it('should validate email format', () => {
      // Arrange
      const validator = BusinessRuleValidator.create<TestData>()
        .apply(coreRules.email('email', 'Invalid email address'));
  ...
  describe('Specification-based rules', () => {
    it('should validate with satisfies', () => {
      ...
      const validator = BusinessRuleValidator.create<TestData>()
        .apply(coreRules.satisfies(isAdult, 'Must be 18 or older'));
    ...
    it('should validate with propertySatisfies', () => {
      ...
      const validator = BusinessRuleValidator.create<TestData>()
        .apply(coreRules.propertySatisfies(
          'name',
          isLongEnough,
  ...
  describe('Conditional rules', () => {
    it('should apply rules conditionally with when', () => {
      // Arrange
      const validator = BusinessRuleValidator.create<TestData>()
        .apply(coreRules.when(
          data => data.premium === true,
          validator => validator.apply(coreRules.minLength('name', 5, 'Premium users need longer names'))
        ));
      
      // Act
      const regularResult = validator.validate(validData);
      const premiumValidResult = validator.validate({...validData, premium: true});
      const premiumInvalidResult = validator.validate({...invalidData, name: 'Jane', premium: true});
      ...
      expect(premiumInvalidResult.error.errors[0].message).toBe('Premium users need longer names');
    });
    ...
    it('should apply otherwise rules', () => {
      ...
      .when(
        data => data.premium === true,
        validator => validator.addRule('name', user => user.name.length >= 5, 'Premium users need longer names')
      )
      .otherwise(
        validator => validator.addRule('name', user => user.name.length >= 2, 'Regular users need at least 2 chars')
      );
      
      // Act
      const regularValidResult = validator.validate({...validData, name: 'Jo'});
      const regularInvalidResult = validator.validate({...invalidData, name: 'J'});
      const premiumValidResult = validator.validate({...validData, premium: true});
      const premiumInvalidResult = validator.validate({...invalidData, name: 'Jane', premium: true});
      ...
      expect(regularInvalidResult.error.errors[0].message).toBe('Regular users need at least 2 chars');
      expect(premiumValidResult.isSuccess).toBe(true);
      expect(premiumInvalidResult.isFailure).toBe(true);
      expect(premiumInvalidResult.error.errors[0].message).toBe('Premium users need longer names');
    });
...
describe('RulesRegistry', () => {
  ...
  beforeEach(() => {
    ...
    // @ts-ignore access to private static field for testing
