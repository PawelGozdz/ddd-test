...
const hasPremiumName = Specification.create<User>(
  (user) => user.name.length >= 3,
);
  ...
  .whenSatisfies(
    ...
    (validator) => {
      validator
        ...
        .addRule(
          'email',
          (user) => user.email.includes('premium'),
          'Premium users need premium email domain',
        );
    },
  )
...
const hasPositiveQuantity = Specification.create<OrderItem>(
...
const isPendingOrConfirmed = Specification.create<Order>((order) =>
...
const orderValidator = BusinessRuleValidator.create<Order>()
  ...
  .mustSatisfy(
    hasValidTotalAmount,
    'Order total amount is inconsistent with item prices',
  )
  .addRule(
    'userId',
  ...
  .whenSatisfies(isPendingOrConfirmed, (validator) =>
    validator.addRule(
      'createdAt',
      (order) => {
        const now = new Date();
...
const hasModerateCreditScore = Specification.create<LoanApplication>(
  (app) => app.creditScore >= 600 && app.creditScore < 700,
);
const hasReasonableLoanAmount = Specification.create<LoanApplication>(
  (app) => app.amount <= app.income * 0.5,
);
const hasNoExistingLoan = Specification.create<LoanApplication>(
  (app) => !app.hasExistingLoan,
);
const hasReasonableTerm = Specification.create<LoanApplication>(
...
const loanApplicationValidator =
  SpecificationValidator.create<LoanApplication>()
    .addRule(
      hasMinimumIncome,
      'Applicant must have minimum annual income of $30,000',
      'income',
    )
    .addRule(
      hasReasonableLoanAmount,
...
const evaluateLoanApplication = (
  ...
  if (validationResult.isFailure) {
    return Result.fail(
      `Invalid application: ${validationResult.error.message}`,
    );
  }
...
const validateUserWithDeepPath = (user: User): Result<User, string> => {
  ...
  if (user.address && user.address.country === 'US') {
    const zipValidator = BusinessRuleValidator.create<string>().addRule(
      '',
      (zip) => /^\d{5}(-\d{4})?$/.test(zip),
      'US ZIP codes must be in format 12345 or 12345-6789',
    );
...
const demoValidation = () => {
  ...
  console.log(
    ...
    basicUserValidator.validate(validUser).isSuccess,
  );
  console.log(
    'Specification-based validation result:',
    specBasedUserValidator.validate(validUser).isSuccess,
  );

  const invalidResult = complexUserValidator.validate(invalidUser);
  console.log(
    'Complex validation failed with errors:',
    invalidResult.isFailure
      ? invalidResult.error.errors.map((e) => e.toString())
      : 'No errors',
  );
  ...
  console.log(
    ...
    loanResult.isSuccess ? loanResult.value : loanResult.error,
  );
};
