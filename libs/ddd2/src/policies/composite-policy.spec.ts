...
describe('CompositePolicy', () => {
  ...
  const createTestPolicy = (
    ...
    const spec = createMock({isSatisfiedBy: () => isSatisfied,}) as ISpecification<any>;
  ...
  describe('check', () => {
    ...
    it('should stop checking at first failure for AND operator', () => {
      ...
      const checkSpy2 = vi.fn().mockReturnValue(Result.fail(new PolicyViolation('CODE2', 'Message 2')));
      const checkSpy3 = vi.fn().mockReturnValue(Result.ok({}));
      
      const mockPolicy1 = { check: checkSpy1, isSatisfiedBy: vi.fn(), and: vi.fn(), or: vi.fn() };
      const mockPolicy2 = { check: checkSpy2, isSatisfiedBy: vi.fn(), and: vi.fn(), or: vi.fn() };
      const mockPolicy3 = { check: checkSpy3, isSatisfiedBy: vi.fn(), and: vi.fn(), or: vi.fn() };
      
      const composite = new CompositePolicy('AND', [mockPolicy1, mockPolicy2, mockPolicy3]);
      const entity = { value: 'test' };
    ...
    it('should check all policies for OR operator', () => {
      // Arrange
      const checkSpy1 = vi.fn().mockReturnValue(Result.fail(new PolicyViolation('CODE1', 'Message 1')));
      const checkSpy2 = vi.fn().mockReturnValue(Result.fail(new PolicyViolation('CODE2', 'Message 2')));
      const checkSpy3 = vi.fn().mockReturnValue(Result.fail(new PolicyViolation('CODE3', 'Message 3')));
      
      const mockPolicy1 = { check: checkSpy1, isSatisfiedBy: vi.fn(), and: vi.fn(), or: vi.fn() };
      const mockPolicy2 = { check: checkSpy2, isSatisfiedBy: vi.fn(), and: vi.fn(), or: vi.fn() };
      const mockPolicy3 = { check: checkSpy3, isSatisfiedBy: vi.fn(), and: vi.fn(), or: vi.fn() };
      
      const composite = new CompositePolicy('OR', [mockPolicy1, mockPolicy2, mockPolicy3]);
      const entity = { value: 'test' };
