...
describe('Business Policy Integration', () => {
  ...
  describe('Policy composition', () => {
    it('should combine policies using AND logic', () => {
      ...
      const activeSpec = Specification.create<User>(
    ...
    it('should combine policies using OR logic', () => {
      ...
      const premiumSpec = Specification.propertyBetween<User>(
        'age',
        25,
        Infinity,
      );
      ...
      expect(result.error.message).toBe(
        'User must be at least 25 years old for premium OR User must be active',
      );
    });
    ...
    it('should handle complex policy chains', () => {
      ...
      const seniorSpec = Specification.propertyBetween<User>(
        'age',
        65,
        Infinity,
      );
      const activeSpec = Specification.propertyEquals<User>('isActive', true);
      const validEmailSpec = Specification.create<User>((user) =>
        user.email.includes('@'),
      );
      ...
      // Assert
      const validYoung: User = {
        age: 20,
        email: 'test@example.com',
        isActive: true,
      };
      const validSenior: User = {
        age: 70,
      ...
      const inactiveAdult: User = {
        age: 20,
        email: 'test@example.com',
        isActive: false,
      };
      const invalidEmailSenior: User = {
        age: 70,
        email: 'invalid',
        isActive: true,
      };
  ...
  describe('Policy Factory and Registry integration', () => {
    it('should facilitate domain-specific policy management', () => {
      ...
      expect(productFactory.get('validPrice').isSatisfiedBy(product)).toBe(
        true,
      );
