...
describe('PolicyRegistry', () => {
  ...
  describe('getPolicy', () => {
    ...
    it('should throw error for non-existent domain', () => {
      ...
      // Act
      const [error] = safeRun(() =>
        PolicyRegistry.getPolicy(nonExistentDomain, 'anyPolicy'),
      );
    ...
    it('should throw error for non-existent policy', () => {
      ...
      // Act
      const [error] = safeRun(() =>
        PolicyRegistry.getPolicy(domain, nonExistentPolicyName),
      );

      // Assert
      expect(error.message).toBe(
        `Policy "${nonExistentPolicyName}" not found in domain "${domain}"`,
      );
    });
  });
  ...
  describe('getDomainPolicies', () => {
    ...
    it('should throw error for non-existent domain', () => {
      ...
      // Act
      const [error] = safeRun(() =>
        PolicyRegistry.getDomainPolicies(nonExistentDomain),
      );
