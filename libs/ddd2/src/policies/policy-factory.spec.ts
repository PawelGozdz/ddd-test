...
import { createPolicyFactory, PolicyRegistry, PolicyViolation } from '../policies';
...
describe('PolicyFactory', () => {
  ...
  const createSpecification = (isSatisfied: boolean): ISpecification<any> => (createMock<ISpecification<any>>({
  ...
  describe('get', () => {
    ...
    it('should throw error for non-existent policy', () => {
      ...
      // Assert
      expect(error.message).toBe(`Policy "${nonExistentPolicy}" not found in domain "${domain}"`);
    });
  });
