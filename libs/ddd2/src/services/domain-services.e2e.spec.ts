...
import { EventAwareDomainService, IBaseDomainService, UnitOfWorkAwareDomainService } from './base-domain-service';
...
class NotificationService extends EventAwareDomainService implements IAsyncDomainService {
  ...
  async initialize(): Promise<void> {
    ...
    this.eventBus.subscribe<OrderConfirmedEvent>(
      OrderConfirmedEvent,
      (event) => this.handleOrderConfirmedEvent(event)
    );
    
    this.eventBus.subscribe<OrderShippedEvent>(
      OrderShippedEvent,
      (event) => this.handleOrderShippedEvent(event)
    );
  }
...
describe('Domain Services - End-to-End Tests', () => {
  ...
  describe('Service Container Dependencies', () => {
    it('should properly resolve service dependencies', async () => {
      ...
      complexContainer.registerFactory('domain-validation', () => new DomainValidationService(), ['infrastructure']);
      complexContainer.registerFactory('order-processor', () => new OrderProcessingService(), ['domain-validation', 'infrastructure']);
  ...
  describe('Event Subscription and Publishing', () => {
    it('should handle event subscription and publishing properly', async () => {
      ...
      eventBus.subscribe(ProductCreatedEvent, (event) => eventLogger.logEvent(event));
      eventBus.subscribe(CustomerRegisteredEvent, (event) => eventLogger.logEvent(event));
      eventBus.subscribe(OrderCreatedEvent, (event) => eventLogger.logEvent(event));
      
      // Act: Execute operations that generate events
      const product = await productService.createProduct('Event Test Product', 199.99);
      const customer = await customerService.registerCustomer('event@test.com', 'Event Tester');
      const order = await orderService.createOrder(customer.getId());
