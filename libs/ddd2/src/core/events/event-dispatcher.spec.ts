...
class MockEventBus {
  ...
  async publish(event: IDomainEvent): Promise<void> {
    if (
      this.shouldFail ||
      (this.failOnEventType && event.eventType === this.failOnEventType)
    ) {
...
describe('EventDispatcher', () => {
  ...
  describe('dispatchEventsForAggregate', () => {
    ...
    it('should throw an error if event publishing fails', async () => {
      ...
      const [error] = await safeRun(() =>
        eventDispatcher.dispatchEventsForAggregate(aggregate),
      );
  ...
  describe('dispatchEvents', () => {
    ...
    it('should throw an error if event publishing fails', async () => {
      ...
      // Act & Assert
      const [error] = await safeRun(() =>
        eventDispatcher.dispatchEvents(event1, event2, event3),
      );
