...
class TestAggregate extends AggregateRoot<string, TestAggregateState> {
  ...
  protected onNameChanged_v2(payload: { name: string; timestamp?: Date }): void {
    this._name = payload.name;
...
class NameChangedUpcaster implements IEventUpcaster<{ name: string }, { name: string; timestamp: Date }> {
  upcast(payload: { name: string }, metadata: any): { name: string; timestamp: Date } {
    return {
      name: payload.name,
      timestamp: new Date(),
    };
...
describe('AggregateRoot', () => {
  ...
  describe('Basic functionality', () => {
    ...
    it('should correctly initialize aggregate with identifier and version', () => {
      ...
      // Act
      const aggregateWithVersion = new TestAggregate({ id: aggregateId, version: initialVersion });
    ...
    it('should correctly check version', () => {
      ...
      const [errorForLowerVersion] = safeRun(() => aggregateWithVersion.checkVersion(3));
  ...
  describe('Snapshotting', () => {
    ...
    it('should throw exception when trying to restore snapshot without enabled feature', () => {
      ...
      // Act
      const [error] = safeRun(() => aggregate.restoreFromSnapshot(mockSnapshot));
    ...
    it('should throw exception when trying to restore snapshot with incorrect ID', () => {
      ...
      // Act
      const [error] = safeRun(() => aggregate.restoreFromSnapshot(wrongIdSnapshot));
    ...
    it('should throw exception when trying to restore snapshot with incorrect type', () => {
      ...
      // Act
      const [error] = safeRun(() => aggregate.restoreFromSnapshot(wrongTypeSnapshot));
    ...
    it('should throw exception when trying to restore invalid snapshot', () => {
      ...
      // Act
      const [error] = safeRun(() => aggregate.restoreFromSnapshot(invalidSnapshot));
  ...
  describe('Versioning', () => {
    ...
    it('should throw exception when trying to register upcaster without enabled feature', () => {
      ...
      // Act
      const [error] = safeRun(() => aggregate.registerUpcaster('NameChanged', 1, upcaster));
    ...
    it('should throw exception when missing appropriate upcaster', () => {
      ...
      const [error] = safeRun(() => (aggregate as any)._upcastEvent(historyEvent));
      expect(error).not.toBeNull();
      expect(error?.message).toMatch(/Missing upcaster for event NameChanged from version 1 to 2/);
    });
  ...
  describe('Complex scenarios', () => {
    it('should work correctly with snapshots and versioning enabled', () => {
      ...
      aggregate.applyWithVersion({ eventType: 'NameChanged', payload: { name: 'Version 1' } }, 1);
      aggregate.applyWithVersion({ eventType: 'ItemAdded', payload: { item: 'Item 1' } }, 1);
      aggregate.applyWithVersion({ eventType: 'ItemAdded', payload: { item: 'Item 2' } }, 1);
      ...
      // Act - Add new events
      newAggregate.applyWithVersion({ eventType: 'NameChanged', payload: { name: 'Version 2' } }, 2);
      newAggregate.applyWithVersion({ eventType: 'ItemAdded', payload: { item: 'Item 3' } }, 1);
  ...
  describe('Test coverage analysis', () => {
    ...
    it('should test missing methods for snapshot capabilities', () => {
      ...
      const incompleteAggregate = new IncompleteAggregate(EntityId.fromUUID('123e4567-e89b-12d3-a456-426614174000'));
      
      // Act & Assert - Create snapshot should fail
      const [createError] = safeRun(() => incompleteAggregate.createSnapshot());
      expect(createError?.message).toMatch(/Method 'serializeState' must be implemented/);
      ...
      const [restoreError] = safeRun(() => incompleteAggregate.restoreFromSnapshot(mockSnapshot));
      expect(restoreError?.message).toMatch(/Method 'deserializeState' must be implemented/);
    });
    
    it('should test snapshot metadata handling', () => {
      // Arrange
      // Create an aggregate with metadata handling
      class MetadataAggregate extends AggregateRoot<string, any, { metaValue: string }> {
        ...
        protected restoreSnapshotMetadata(metadata: { metaValue: string }): void {
      ...
      const metaAggregate = new MetadataAggregate(EntityId.fromUUID('123e4567-e89b-12d3-a456-426614174000'));
      ...
      const newMetaAggregate = new MetadataAggregate(EntityId.fromUUID('123e4567-e89b-12d3-a456-426614174000'));
